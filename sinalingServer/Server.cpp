...
void Server::handleAuthentication(const SOCKET& clientSocket)
{
   ...
   char numberOfKids = '0';
    ...
    do
    {
        ...
        if (authenticationResponse.status == SUCCESS_SIGNUP) {
            ...
            if (authenticationRequest.role == "Parent")
            {
                ...
                currentState = State::Handle_connections;
            }
            else
            {
                hashedSecret.assign(receivedMsg.begin() + 1, receivedMsg.end());
            ...
            else if (authenticationRequest.role == "Child")
            {
                ...
                auto timeout = std::chrono::seconds(60); // Timeout after 60 seconds

                std::unique_lock<std::mutex> lock(peersMutex); // Assuming 'mutex' protects waitingPeers
    ...
    if (currentState != State::sdp)
    {
        if (authenticationRequest.role == "Parent")
            {
        
                auto startTime = std::chrono::steady_clock::now();
                auto timeout = std::chrono::seconds(300); // max 1 min

                lock.lock();
                auto& children = waitingPeers[hashedSecret]; // מתייחס ישירות לרשימה של ילדים

                while (currentState != State::sdp && (std::chrono::steady_clock::now() - startTime < timeout)) {
                    lock.unlock();
                    if (children.childrenCommunicators.size() != 0)
                    {
    ...
    {
        std::cerr << "timoutttttttt" << std::endl;
    }
