...
private:
	...
	std::pair<uint64_t, uint64_t> clientHandshake(Communicator& comm); // return the random number of client and server. client->first, server->second
	...
	bool sendCertificateMessegeToServer(Communicator& comm);
...
public:
	...
	bool runParentDTLS(Communicator& comm);
